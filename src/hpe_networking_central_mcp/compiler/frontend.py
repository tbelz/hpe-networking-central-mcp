...
This module performs exactly one mutation on the input before handing it
to prance: it strips top-level keys beginning with ``_`` (e.g. ``_id``)
because the ReadMe.io export pipeline injects internal indexing metadata
that strict OAS 3.1 validation rejects via ``unevaluatedProperties:
False``.  No other normalisation is performed here; substantive content
flows through unchanged so the downstream AST generator (Task 2) sees
the original spec.
"""
...
def _strip_underscore_keys(obj: Any) -> Any:
    ...
    ``unevaluatedProperties: False``.  Removing them is the only
    pre-processing this module performs.
    """
...
def resolve_spec(spec: dict[str, Any], *, source: str) -> ResolvedSpec | ResolutionFailure:
    ...
    cleaned = _strip_underscore_keys(spec)
    try:
...
def resolve_specs(specs: list[dict[str, Any]]) -> ResolutionResult:
    ...
    The ``source`` label for each spec is taken from the optional
    ``_spec_source`` tag stamped by the spec providers
    (``ReadMeSpecProvider``, ``GreenLakeSpecProvider``) plus the spec
    title; otherwise the spec's own title is used.  Specs without an
    identifiable source fall back to ``"unknown"``.
    """
    result = ResolutionResult()
    for raw in specs:
        source = raw.get("_spec_source") or _spec_title(raw, "unknown")
        outcome = resolve_spec(raw, source=str(source))
